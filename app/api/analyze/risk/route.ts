...
import { AssetResolverTool, PricesTool, HistoryTool, RiskMetricsTool, AlertsTool } from "@/lib/agent/tools"
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const { portfolio, windowDays = 90, userId = "default" } = body
        ...
        let historyResult
        try {
          historyResult = await historyTool.run({ symbols, windowDays }, ctx)
          ...
          sendEvent({
            type: "error",
            step: "fetch_history",
            message: "Failed to fetch history, using cached data",
            timestamp: Date.now(),
          })
          controller.close()
          return
        ...
        if (validation.eligible.length < 3) {
          sendEvent({
            type: "error",
            step: "validation",
            message: `Insufficient eligible assets (${validation.eligible.length}/3). ${validation.notes.join("; ")}`,
            timestamp: Date.now(),
          })
          controller.close()
          return
        ...
        const totalValue = validation.eligible.reduce((sum, sym) => {
          return sum + holdings[sym] * (pricesResult.data[sym] || 0)
        }, 0)

        const weights: Record<string, number> = {}
        for (const sym of validation.eligible) {
          weights[sym] = (holdings[sym] * (pricesResult.data[sym] || 0)) / totalValue
        }

        sendEvent({
          type: "progress",
          step: "calculate_metrics",
          message: "Computing risk metrics...",
          timestamp: Date.now(),
        })

        const riskTool = new RiskMetricsTool()
        const startRisk = Date.now()
        let riskMetrics
        try {
          riskMetrics = await riskTool.run(
            {
          ...
          console.log(`[INFO] [requestId=${requestId}] step=risk_metrics ms=${Date.now() - startRisk}`)
        } catch (error) {
          console.log(`[ERROR] [requestId=${requestId}] step=risk_metrics code=CALC_FAILED detail=${error}`)
          sendEvent({
            type: "error",
            step: "calculate_metrics",
        ...
        sendEvent({
          ...
          message: "Checking risk alerts...",
          timestamp: Date.now(),
        })

        const policy = loadPolicy(userId)
        const alertsTool = new AlertsTool()
        const alerts = await alertsTool.run({ risk: riskMetrics, weights, policy }, ctx)
        ...
        console.log(`[INFO] [requestId=${requestId}] step=complete alerts=${alerts.length}`)
        controller.close()
      } catch (error) {
